...
class AllTrackedWeeklylyHabit(Static):
    ...
    def __init__(self,u,apii):
        ...
        self.list_of_habitid = self.hapi.weekly_list_of_a_user_habits(self.userList[0])

        self.listLen=len(self.list_of_habitid)
        self.table = DataTable(id='all_weekly_habit')
    ...
    def compose(self) -> ComposeResult:
        if self.listLen==0:
            yield ZeroDisplay("No Weekly habit registered yet")
        else:
            #self.myVal=self.query_one(Who2).who2
            yield Label('_________________________________________________________________________________________________________')
            yield Header1('ALL CURRENTLY TRACKED Weekly HABITS')
            yield self.table
            yield Label('_________________________________________________________________________________________________________')
    ...
    def on_mount(self) -> None:
        if self.listLen == 0:
            ...
            table = self.query_one('#all_weekly_habit')
